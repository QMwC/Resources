locations = [
  ...
  [152.5, 0, 0],   // Rotator
  [350, 0, 0],     // ScreenBlock
  [350, 0, -355],  // MirrorPathReflect
  [705, 0, 0]      // MirrorPathTransmit
]

let obj_right = 100;
let dist_away = 350;
let ang = Math.PI/5;
let moving_x = true;  // Whether the photon's moving in x or z direction.


function move(loc) {
  let up = dist_away*Math.sin(ang);
  let away = dist_away*Math.cos(ang);
  p(loc[0]+obj_right*moving_x+away*!moving_x, up, loc[2]+away*moving_x-obj_right*!moving_x);
  a(Math.PI/2*!moving_x, ang);
}


function pton_capture() {
  let up = dist_away*Math.sin(ang);
  let away = dist_away*Math.cos(ang);
  let pton_right = obj_right-50*sign(obj_right);

  a(Math.PI/2*!moving_x, ang);
}

//p(-355+150*Math.cos(ang), 150*Math.sin(ang), 0); a(PI/2, ang);  // CAPTURE SCREEN
